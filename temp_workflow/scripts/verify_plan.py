...
def verify_single_file(file_path: Path, all_exts: list) -> list:
    ...
    # 2. 檢查 Header 元數據
    header_block = "\n".join(lines[:25])
    has_name = bool(re.search(r">\s*(功能名稱|計畫名稱)：", header_block))
    has_date = bool(re.search(r">\s*(建立日期|完成日期)：", header_block))
    has_status = bool(re.search(r">\s*(狀態|Status)：", header_block))
    has_ext = bool(re.search(r">\s*(擴充項目|active ext\.?)：", header_block))
    has_version = bool(re.search(r">\s*(模板版本|Version)：", header_block))
    ...
    ext_line_match = re.search(r">\s*(擴充項目|active ext\.?)：(.*)", header_block)
    declared_exts_text = ext_line_match.group(2).strip() if ext_line_match else ""
