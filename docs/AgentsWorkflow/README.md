...
source_paths:
  ...
  - "source/agents-workflow/config.template.json"
...
支援一鍵在專案 `.agents/workflows/` 生成對應 IDE 格式的引用式指令文件，並提供自動舊檔覆寫清理與一鍵重置指令：

```bash
# 生成預設指令 (例如 NewPlan.md)
python yscb_cli.py agents-workflow --ide-gemini

# 附帶 sop_ 前綴生成 (例如 sop_NewPlan.md，生成前自動清理先前產出的指令)
...
# 一鍵清理所有由 IDE 生成器產生的指令並重置 config.json
python yscb_cli.py agents-workflow --ide-clear
```

- **自動清理 (Pre-Generation Cleanup)**：調用 `--ide-gemini` 時自動檢查 `config.json`，若存在歷史生成紀錄，先精準移除舊指令（如切換前綴 `custom_` $\rightarrow$ `sop_` 時自動清除舊的 `custom_*.md`），防止孤兒檔案殘留。
- **引用式設計**：生成的指令檔案僅包含 YAML Frontmatter 描述與指向核心工作流的相對路徑連結，保證 Single Source of Truth (SSOT)。
- **設定留痕**：相關生成目錄、前綴、檔案清單與時間戳自動持久化記錄於模組 `config.json`。
