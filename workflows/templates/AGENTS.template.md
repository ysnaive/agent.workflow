...
- **目錄歸檔紀律與腳本優先**：
  - **定式作業腳本優先**：進行歷史 Dev Plan / DR 檢索、狀態掃描、合規驗證或計畫歸檔等定式作業時，必須優先呼叫 `.agents/scripts/` 下的 Python 工具腳本。
...
| :--- | :--- | :--- |
| **新 Session/Chat 啟動上下文** | [sop_ContextInit.md](./workflows/sop_ContextInit.md) | 在沙盒與全權限模式下安全秒級熱啟動專案規範與歷史脈絡 |
| **開始新功能開發 / 重大修改** | [sop_NewPlan.md](./workflows/sop_NewPlan.md) | 從 Phase 0 開始執行完整的 SOP 分析與三大分流規劃 |
| **接續中斷或已存在的計畫** | [sop_Continue.md](./workflows/sop_Continue.md) | 自動掃描 `.agents/dev_plans/`，偵測 `handoff.md` 並恢復進度 |
| **大型/跨度大的深度技術調研** | [sop_Research.md](./workflows/sop_Research.md) | 深度技術探討、業界方案對比與 `R01_xxx.md` 報告產出 |
| **開發遇阻/根因排查** | [sop_Discuss.md](./workflows/sop_Discuss.md) | 強制停手、5-Whys 根因分析、防範排查越界與淺層修補 |
| **暫停開發/現場凍結交接** | [sop_Pause.md](./workflows/sop_Pause.md) | 於計畫目錄生成 `handoff.md` 現場狀態，保證零斷層接手 |
| **開發完成後品質與細節審查** | [sop_Review.md](./workflows/sop_Review.md) | 調用 `verify_plan.py`、全量 Extension 深度稽核與即時修復閉環 |
| **構想與靈感孵化池** | [sop_Idea.md](./workflows/sop_Idea.md) | 開放式自由探討，產出 What/Why/How/Related 提案書 |
| **更新 / 檢索專案知識庫文檔** | [DocumentationStandards.md](./workflows/DocumentationStandards.md) | 遵循 Package-like 鏡像結構維護 `docs/` |
<!-- CORE_RULES_END -->

## 3. 專案特化工程規範 (Project Specific Standards)
